...
export function TransactionsClient({ initialTransactions, companies }: { initialTransactions: TransactionData[], companies: CompanyData[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return transactions.filter(txn => {
      ...
      const matchSearch = !q
        ...
        || ((txn as any).vendor_name || "").toLowerCase().includes(q)
  ...
  const filters: FilterConfig[] = [
    ...
    },
    {
      ...
      options: Object.entries(CATEGORY_LABELS).map(([k, v]) => ({ label: v as string, value: k }))
    },
    {
  ...
  const columns: ColumnDef<TransactionData>[] = [
    ...
    { key: "description", label: "Description", sortable: false, render: (row) => <span className="max-w-[150px] truncate block text-xs">{row.description || (row as any).notes || "—"}</span> },
